            ...
            if self.safe_visible(ans):
                ...
                if self.safe_visible(captcha_box):
                    ...
                    if solved_code:
                        ...
                        self.fill_field(ans, solved_code)
                        page.click("#jar")
        ...
        try:
            ...
            try:
                if not nid_input.input_value():
                    self.fill_field(nid_input, self.nid)
            except Exception:
                self.fill_field(nid_input, self.nid)

            try:
                if not track_input.input_value():
                    self.fill_field(track_input, tracking_code)
            except Exception:
                self.fill_field(track_input, tracking_code)
            ...
            try:
                if not self.fill_field(captcha_input, captcha_value):
                    raise RuntimeError("captcha field verification failed")
