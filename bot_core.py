...
from otp_session_manager import OtpSessionManager
...
class BotCore:
    def __init__(self, nid, settings, log_callback=None, captcha_service=None, browser_profile=None):
        ...
        self.interaction_config = InteractionConfig.from_settings(settings)
        ...
        self.otp_relay = OtpRelayClient.from_env()
        self.otp_sessions = OtpSessionManager(self.otp_relay)
        self.otp_job_id = str(self.settings.get("job_id") or uuid.uuid4())
        self.otp_purpose = str(self.settings.get("otp_purpose") or "register")
        self.legacy_otp_fallback = os.getenv("MAHAN_OTP_LEGACY_FALLBACK", "false").lower() in {"1", "true", "yes"}
    ...
    def fill_field(self, locator, value, *, blur_after=True) -> bool:
        """Fill a text field using the selected human/fast interaction mode."""

        return BrowserActions.fill_text(
            locator,
            value,
    ...
    def get_otp_code(self, stop_event=None, timeout: int = 45):
        """Return only an OTP received after this applicant entered the OTP step."""

        # The active Job owns exactly one relay session.
        try:
            code = self.wait_for_otp(stop_event, timeout=timeout)
            if code:
                self.log("OTP received for active session: ******", "success")
                return str(code).strip()
        except Exception:
            pass

        if not self.legacy_otp_fallback:
            return None
        # Temporary v1 migration fallback, disabled by default.
        try:
            record = DBHandler.get_otp_record(self.nid) or {}
            code = str(record.get("otp") or "").strip()
            record_status = str(record.get("status") or "").strip().lower()
            record_ts = float(record.get("ts") or 0.0)
            if (
                record_status == "received"
                and record_ts > float(self._otp_entry_time or 0.0)
                and code.isdigit()
                and 4 <= len(code) <= 8
            ):
                return code
    ...
    def mark_otp_entry_time(self, ts: float = None) -> float:
        ...
        if self.otp_relay.configured:
            try:
                self.otp_sessions.create(job_id=self.otp_job_id, national_id=self.nid,
                                         purpose=self.otp_purpose)
            except Exception:
                self.log("OTP relay is temporarily unreachable while opening the wait window.", "warning")
                raise
        elif not self.legacy_otp_fallback:
            raise RuntimeError("OTP Relay v2 is not configured")
        else:
            DBHandler.mark_otp_waiting(self.nid, self._otp_entry_time)
    ...
    def wait_for_otp(self, stop_event=None, timeout: int = 45, min_ts: float = None):
        ...
        if self.otp_relay.configured:
            return self.otp_sessions.wait(self.otp_job_id, timeout, stop_event)
        if self.legacy_otp_fallback:
            record = DBHandler.get_otp_record(self.nid) or {}
            code = str(record.get("otp") or "").strip()
            record_ts = float(record.get("ts") or 0.0)
            if record_ts > self._otp_entry_time and code.isdigit() and 4 <= len(code) <= 8:
                return code
        return None
    ...
    def clear_otp_backend(self):
        if self.otp_relay.configured:
            try:
                self.otp_sessions.cancel(self.otp_job_id)
            except Exception:
                pass
        if self.legacy_otp_fallback:
            try:
    ...
    def mark_otp_consumed(self):
        if self.otp_relay.configured:
            try:
                self.otp_sessions.consume(self.otp_job_id)
                self.log("OTP consumed successfully", "success")
            except Exception:
                pass
        if self.legacy_otp_fallback:
            try:
                DBHandler.mark_otp_consumed(self.nid)
            except Exception:
                pass
        ...
        try:
            ...
            if firewall_input.count() > 0 and firewall_input.first.is_visible():
                ...
                if imgs:
                    ...
                    if code:
                        self.fill_field(firewall_input.first, code)
