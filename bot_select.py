...
class BankSelectionBot(BotCore):
    ...
    def fetch_otp_fast(self, stop_event=None, timeout: int = 120, min_ts: float = 0.0):
        ...
        otp_code = self.wait_for_otp(stop_event=stop_event, timeout=timeout, min_ts=min_ts)
        if otp_code:
            ...
            if input_sel:
                try:
                    ...
                    if target.count() > 0 and target.first.is_visible():
                        self.fill_field(target.first, "", blur_after=False)
                except Exception:
                    pass
            return True
        except Exception:
        ...
        for selector in selectors:
            try:
                ...
                try:
                    ...
                    current = ""
                if not current:
                    self.fill_field(field.first, self.nid)
                    self._nid_filled = True
                return True
            except Exception:
                continue
    ...
    def fast_otp_sync_and_inject(self, page, stop_event):
        """
        Direct cycle: HTTPS relay -> local DB -> browser.
        ...
        self._otp_guard_until = time.time() + 120.0
        self.log("Direct OTP sync active; waiting for SMS through HTTPS relay.", "info", page)
        ...
        try:
            while not stop_event.is_set():
                ...
                if self.otp_relay.configured:
                    # Pull the remote HTTPS relay into the local applicant record.
                    self.wait_for_otp(
                ...
                otp_code = str(record.get("otp") or "").strip()
                try:
                    otp_ts = float(record.get("ts") or 0.0)
                except (TypeError, ValueError):
                    otp_ts = 0.0
                if (
                    str(record.get("status") or "").lower() == "received"
                    and otp_ts > self._otp_entry_time
                    and re.fullmatch(r"\d{4,8}", otp_code)
                ):
                    self.log("OTP تازه از گوشی دریافت شد.", "success", page)
                    break
            ...
            if not self.fill_field(page.locator(otp_selector), otp_code):
                return None
    ...
    def _fill_input_with_verification(self, page, locator, value, label):
        if self.fill_field(locator, value):
    ...
    def _solve_captcha_wrapper(self, page, input_sel, btn_sel, mode):
        try:
            while True:
                ...
                try:
                    self.fill_field(inp.first, "", blur_after=False)
                except Exception:
                    pass
    ...
    def solve_captcha_step(self, page, input_sel: str, btn_sel: str, source: str = "general") -> bool:
        ...
        try:
            ...
            inp = page.locator(input_sel)
            if not self.fill_field(inp, code):
                return False
            page.locator(btn_sel).click()
    ...
    def _perform_login_standard(self, page, mode):
        try:
            if not page.locator("#ctl00_ContentPlaceHolder1_tbIDNo").input_value():
                self.fill_field(page.locator("#ctl00_ContentPlaceHolder1_tbIDNo"), self.nid)
