...
def _prepare_environment() -> Dict[str, str]:
    ...
    values.setdefault("MAHANBOT_INTERACTION_MODE", "human")
    values.setdefault("MAHANBOT_HUMAN_TYPING_DELAY_MS", values.get("MAHANBOT_TYPING_DELAY_MS", "70"))
    values.setdefault("MAHANBOT_FAST_TYPING_DELAY_MS", "0")
    values.setdefault("MAHAN_OTP_RELAY_URL", "https://otp.mahanvip.ir")
    values.setdefault("MAHAN_OTP_BOT_KEY", "")
    values.setdefault("MAHAN_OTP_PHONE_KEY", "")
    values.setdefault("MAHAN_OTP_WAIT_TTL_SECONDS", "600")
    values.setdefault("MAHAN_OTP_LONG_POLL_SECONDS", "20")
...
def _write_phone_setup(env: Dict[str, str]) -> str:
    relay_url = str(env.get("MAHAN_OTP_RELAY_URL") or "").strip().rstrip("/")
    phone_key = str(env.get("MAHAN_OTP_PHONE_KEY") or "").strip()
    if relay_url and phone_key:
        base_url = relay_url
        endpoint = f"{base_url}/api/v1/sms/otp"
        status_url = f"{endpoint}/status"
        mode = "HTTPS relay (remote phone supported)"
        device_key = phone_key
    else:
        ip = _lan_ip()
        base_url = f"http://{ip}:{env['MAHANBOT_SMS_PORT']}"
        endpoint = f"{base_url}/api/v1/sms/otp"
        status_url = f"{endpoint}/status"
        mode = "Local LAN fallback"
        device_key = env['MAHANBOT_SMS_DEVICE_KEY']

    content = (
        "MahanBot - SMSForwardManager OTP setup\n"
        "==========================================\n"
        f"Mode: {mode}\n"
        f"MahanBot address: {base_url}\n"
        f"OTP endpoint: {endpoint}\n"
        f"Status endpoint: {status_url}\n"
        f"Device key: {device_key}\n\n"
        "The Android app may be on another network when HTTPS relay mode is configured.\n"
        "Enable OTP transfer only with the phone owner's explicit consent.\n"
        "Only national ID, extracted OTP, time, and opaque device/message IDs are transferred.\n"
        "The full SMS body and sender are not accepted by the relay.\n"
        "Keep both phone and bot keys private.\n"
    )
...
def main() -> int:
    ...
    print(f"Android OTP endpoint: {phone_endpoint}")
