...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if(document.getElementById('otpInput')){
        document.getElementById('otpInput').addEventListener('keypress', function (e) {
            if (e.key === 'Enter') sendManualOtp();
        });
    }

    document.querySelectorAll('input[name="otpMode"]').forEach(function (input) {
        input.addEventListener('change', updateOtpModeUi);
    });
    updateOtpModeUi();
...
function updateOtpModeUi() {
    const manual = document.getElementById('otpModeManual')?.checked === true;
    document.getElementById('otpAutoPanel')?.classList.toggle('d-none', manual);
    document.getElementById('otpManualPanel')?.classList.toggle('d-none', !manual);
    const statusEl = document.getElementById('otpStatus');
    if (statusEl) {
        statusEl.textContent = manual ? '' : 'در انتظار دریافت خودکار از اپ اندروید...';
    }
    if (manual) document.getElementById('otpInput')?.focus();
}
...
async function sendManualOtp() {
    ...
    if (!nid) {
        if (statusEl) statusEl.innerHTML = '<span class="text-danger">متقاضی فعالی انتخاب نشده است.</span>';
        return;
    }
    ...
    if (code.length < 4 || code.length > 8) {
        if (statusEl) statusEl.innerHTML = '<span class="text-danger">کد باید بین ۴ تا ۸ رقم باشد.</span>';
        return;
    }

    if (statusEl) statusEl.textContent = 'در حال ثبت...';

    await apiCall(`/manual_otp`, 'POST', {nid: nid, code: code});

    if (statusEl) statusEl.innerHTML = '<span class="text-success">کد دستی با موفقیت ثبت شد.</span>';
...
async function loadSettings() {
    ...
    try {
        ...
        document.getElementById('set_interaction_mode').value = s.interaction_mode || 'human';

        document.getElementById('set_human_typing_delay').value = s.human_typing_delay_ms ?? 70;
...
async function saveSettings() {
    ...
    const payload = {
        ...
        interaction_mode: document.getElementById('set_interaction_mode').value,

        human_typing_delay_ms: parseInt(document.getElementById('set_human_typing_delay').value || '70', 10),
