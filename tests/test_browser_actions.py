...
class BrowserActionsTests(unittest.TestCase):
    ...
    def test_fast_mode_uses_immediate_fill_and_verifies_value(self):
        locator = FakeLocator(value="old")
        ...
        result = BrowserActions.fill_text(locator, "123456", config=config)

        self.assertTrue(result)
        self.assertEqual(locator.value, "123456")
        self.assertTrue(any(event[0] == "fill" for event in locator.events))
        self.assertFalse(any(event[0] == "type" for event in locator.events))

    def test_config_from_settings_selects_human_and_fast_modes(self):
        human = InteractionConfig.from_settings(
            {"interaction_mode": "human", "human_typing_delay_ms": 95}
        )
        fast = InteractionConfig.from_settings(
            {"interaction_mode": "fast", "fast_typing_delay_ms": 0}
        )

        self.assertEqual(human.mode, "human")
        self.assertEqual(human.typing_delay_ms, 95)
        self.assertEqual(fast.mode, "fast")
        self.assertEqual(fast.typing_delay_ms, 0)
