...
class UnifiedLauncherTests(unittest.TestCase):
    ...
    def test_phone_setup_contains_consented_otp_endpoint(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                self.assertIn("/api/v1/sms/otp", endpoint)
                self.assertIn("explicit consent", text)
                self.assertIn("full SMS body and sender are not accepted", text)
