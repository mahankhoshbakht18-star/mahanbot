...
class UiV2Tests(unittest.TestCase):
    def test_dashboard_has_one_otp_input_with_manual_and_auto_modes(self):
        html = (Path(__file__).parent.parent / 'static' / 'index.html').read_text(encoding='utf-8')
        self.assertEqual(html.count('id="otpInput"'), 1)
        self.assertNotIn('id="manualOtpInput"', html)
        self.assertIn('id="otpModeAuto"', html)
        self.assertIn('id="otpModeManual"', html)
